...
def available_test_user():
    test_user = {"username": "test09001", "email": "test0900@gmail.com", "password": "12test12"}
    yield test_user
...
def test_registration_redirect_without_filled_fields(browser):
    browser.find_element(By.LINK_TEXT, "Register").click()
    WebDriverWait(browser, timeout=3).until(lambda driver: driver.find_element(By.NAME, "username")).send_keys(
        "test_user"
    )
    browser.find_element(By.XPATH, "//button[@type='submit']").click()
    try:
        WebDriverWait(browser, timeout=3).until(EC.url_to_be("https://127.0.0.1:8001/login/"))
...
def test_validate_bootstrap_field_validation_message(browser):
    ...
    assert "Please fill out this field." in email_field.get_attribute("validationMessage")
...
def test_register_new_student_user_and_login_logout_as_new_user(browser, test_user_credentials):
    ...
    WebDriverWait(browser, timeout=3).until(lambda driver: driver.find_element(By.LINK_TEXT, "Logout")).click()
